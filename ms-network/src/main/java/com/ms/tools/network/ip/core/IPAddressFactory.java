...
import com.ms.tools.network.domain.DomainUtils;
import com.ms.tools.network.okhttp.download.DownloadUtils;
    ...
    private void init(String filePath) {
        try {
            ...
            try {
                ...
                String msg = StrFormatUtils.format("数据文件未找到，请前往官网下载：" + e.getMessage());
                log.warning(msg);
            ...
            if (ipFile != null) {
                try {
                    ...
                    String msg = StrFormatUtils.format("文件格式异常:" + e.getMessage());
                    log.warning(msg);
                    ipFile = null;
                }
            }
    ...
    public String getCity(String ipAddress) {
        try {
            if (ipAddress.startsWith("192.168.")) {
                log.warning(StrFormatUtils.format("此IP[{}]段不进行处理！", ipAddress));
                return null;
            }
            return getIPLocation(ipAddress).getCity();
        } catch (Exception e) {
            log.warning(StrFormatUtils.format("根据IP[{}]获取省份失败:{}", ipAddress, e.getMessage()));
            return null;
        }
    }
}
